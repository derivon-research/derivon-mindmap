...
the learner and the workspace.** The concrete layout is one shared specification:

```text
<application data directory>/learner-records/<workspace id>/
├── state.json    # derivon.learning/v1
└── routes.json   # derivon.routes/v1
```

- **The purpose-built workspace `id` from `derivon.workspace/v1` is the key.** It is a
  user-chosen, immutable, filesystem-safe path segment. Its equality is identity: the same id
  in two manifests is the same workspace and the same record, and copying a workspace keeps
  the record rather than forking it.
...
- **The index from `id` to the last path it was seen at is not part of the records.** It is
  ...
  same id appearing at a new path and to offer migration, never to drop a record silently.
...
- Closing or crashing the application still does not recover anything that was not written;
  what differs from the session-local phase is that the write path now exists and the learner's
  own explicit actions use it.
- Workspace identity is now load-bearing for persistence, so the id validation in
  ...
  content workspaces are migrated in one group.
