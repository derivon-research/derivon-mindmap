...
A repeated acquisition of that revision reads only what moved. The cheap signal compared before any read is a file's size, its modification time, and — where the platform reports one — its inode change time. A file whose signal moved is a candidate and has its bytes read; a file whose signal held still keeps the digest last read from its bytes; a path whose kind or listing could not be read is never remembered, because its fingerprint comes from metadata and access state rather than from a read, and is re-derived on every acquisition. The comparison runs over the same traversal as a full verification, so it skips the same root `.git`, refuses the same symlinks, and reports the same unreadable paths, and added, renamed and removed files are compared by path rather than inferred from a directory timestamp.

Cheap-signal equality is an observation, not a proof, and it is not presented as one. A change that leaves size, modification time and change time identical — which takes a writer that restores modification time, on a platform that reports no change time — is not seen by a poll. Nothing is granted on it: every commit verifies the whole source from bytes before it writes, so a change the poll did not see refuses that commit as "workspace changed externally before commit" instead of being overwritten. A poll therefore costs a metadata walk per second and reads only what moved, while the certainty a write requires is still bought once per commit.
