...
own token (its process id and a timestamp taken when it first needed one) and a counter, so two
live writers cannot choose the same name, and a temporary file a crash left behind cannot block a
later replacement. The file is removed on every failure path; when that removal itself fails,
the commit reports that second failure and the leftover stays on disk, where change detection
reads it once as an added file that then holds still. A process that dies mid-commit can leave one
behind the same way, which blocks no later replacement. Because the target becomes a new file, its
permission bits are carried over where they can be read — a target whose metadata cannot be read
keeps the defaults — while another name hard-linked to the old one keeps the old contents. A
target that is a symlink is replaced by a regular file, where an in-place write would have written
through the link: a commit never reaches that case, because revision acquisition refuses a
workspace containing a symlink, but the whole-manifest write that does not acquire a revision
does not check for one. On Windows a
replacement can be refused where an in-place write would have succeeded,
