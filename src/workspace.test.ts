...
describe('authoring workspace', () => {
  it('bundles one Derivon skill for common coding-agent discovery paths', () => {
    ...
    const skills = paths.filter((path) => path.endsWith('/SKILL.md')).map((path) => WORKSPACE_AGENT_FILES[path]);
    expect(new Set(skills).size).toBe(1);
    expect(skills[0]).toContain('name: derivon-workspace');
    expect(skills[0]).toContain('managed-by: derivon-mindmap-demo');
