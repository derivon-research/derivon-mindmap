...
export function AuthoringMode({ active = true, workspace, content, authoring, routeSolver, selectedConceptId, onSelectConcept, syncStatus, onRetrySync, readAsset, readDocuments, conversation }: AuthoringModeProps) {
  ...
  return <section className="authoring-workbench" data-derivon-mode="authoring" data-relations-open={relationsOpen} data-agent-open={agentOpen} data-selected-concept={selectedConceptId ?? ''} aria-label="创作侧">
    ...
    <div className="authoring-workspace">
      ...
      <AuthoringAgentPane open={agentOpen} onToggle={() => setAgentOpen(!agentOpen)} contextLabel={title || workspace.name} conversation={conversation} />
    </div>
