...
export function AuthoringMode({ active = true, workspace, content, authoring, routeSolver, selectedConceptId, onSelectConcept, syncStatus, onRetrySync, readAsset }: AuthoringModeProps) {
  ...
  // Creating a concept is a name and a return key; everything else about it is edited on
  // the object page it opens.
  const create = () => {
    setFormError('');
    if (!newLabel.trim() || !authoring) return;
    try {
      const id = authoring.createConcept({ label: newLabel });
      setNewLabel(''); authoring.protectDraft(draftKey, false);
      setSelected({ kind: 'concept', id }); setNeighbourhoodFocusId(id); onSelectConcept(id); setView('objects');
