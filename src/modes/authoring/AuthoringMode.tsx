...
export function AuthoringMode({ active = true, workspace, content, authoring, routeSolver, selectedConceptId, onSelectConcept, syncStatus, onRetrySync, readAsset, readDocuments }: AuthoringModeProps) {
  ...
  // Deletion opens at the foot of the object page, under the reference report it extends.
  // A plan belongs to the object it was read on, and is discarded with it: the section is
  // remounted per object, so this only ever says whether the current one is open.
  ...
  const deleted = () => {
    setDeleting(false); setSelected(null); setLastOpenedObject(null); setNeighbourhoodFocusId(null);
