...
export function AuthoringMode({ active = true, workspace, content, authoring, selectedConceptId, onSelectConcept, syncStatus, onRetrySync, readAsset }: AuthoringModeProps) {
  ...
  return <section className="authoring-workbench" data-derivon-mode="authoring" data-relations-open={relationsOpen} data-agent-open={agentOpen} data-selected-concept={selectedConceptId ?? ''} aria-label="创作侧">
    ...
    <div className="authoring-workspace">
      ...
      <div className="authoring-content">{view === 'graph' && <main className="authoring-graph-page"><header><div className="authoring-tabs" role="group" aria-label="图视图"><button type="button" aria-pressed={graphKind === 'overview'} onClick={() => setGraphKind('overview')}>全图</button><button type="button" aria-pressed={graphKind === 'neighbourhood'} disabled={!focusId} onClick={() => setGraphKind('neighbourhood')}>关联布局</button></div><span className="authoring-flex" />{selected && <button type="button" onClick={() => setView('objects')}><FileText size={15} />回到对象</button>}</header><div className="authoring-graph">{active && <Suspense fallback={<span role="status">正在载入图…</span>}><GraphRenderer view={graphView} onEvent={graphEvent} /></Suspense>}</div><div className="authoring-graph-caption"><span>{title || '未选择对象'}</span><span>{graphView.concepts.length} 个概念 · {graphView.hyperedges.length} 条推导</span></div></main>
