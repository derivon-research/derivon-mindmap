...
import {
  applyOrientationIntent, beginOrientation, currentQuestion, isOrientationComplete, orientationSeedKnown, planOrientation,
} from './orientation';
...
describe('the orientation plan', () => {
  ...
  it('falls back to the generic entry when no configuration ships with the workspace', () => {
    ...
    expect(beginOrientation(plan)).toEqual({ targets: [], at: -1, trail: [], asked: [], round: 0 });
...
describe('orientation transitions', () => {
  ...
  it('initializes a route from the default seed before any question is answered', () => {
    ...
    expect(orientationSeedKnown(plan())).toEqual(['a']);
  ...
  it('applies a single-select answer, follows its branch and records the trail', () => {
    const p = plan();
    const { run } = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['paper'] });
  ...
  it('expands tags on the way in and unions a multi-select answer', () => {
    const p = plan();
    const answered = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['paper'] });
    const { run, known } = applyOrientationIntent(p, answered.run, answered.known, { kind: 'answer', optionIds: ['basics', 'none'] });
  ...
  it('falls through to the next question in document order when an option names no branch', () => {
    const p = plan();
    const { run } = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['course'] });
    expect(run.targets).toEqual(['c']);
    expect(currentQuestion(p, run)?.id).toBe('known');
  });
  ...
  it('skips a question without changing targets or known', () => {
    ...
    const { run, known } = applyOrientationIntent(p, begun, ['a'], { kind: 'skip' });
    expect({ targets: run.targets, known }).toEqual({ targets: begun.targets, known: ['a'] });
    expect(currentQuestion(p, run)?.id).toBe('known');
  });

  it('rejects an answer from outside the question being asked', () => {
    const p = plan();
    expect(() => applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['basics'] }))
      .toThrow(/basics/);
    expect(() => applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['paper', 'browse'] }))
      .toThrow(/单选/);
  });

  it('takes direct target and known changes, keeping only concepts the graph has', () => {
    const p = plan();
    const { run } = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'set-targets', conceptIds: ['b', 'ghost'] });
    expect(run.targets).toEqual(['b']);
    expect(applyOrientationIntent(p, run, ['a'], { kind: 'set-known', conceptIds: ['ghost'] }).known).toEqual([]);
  });

  it('restarts back to the seed, keeping the configuration', () => {
    const p = plan();
    const answered = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'answer', optionIds: ['paper'] });
    const restarted = applyOrientationIntent(p, answered.run, answered.known, { kind: 'restart' });
    expect(restarted.run).toEqual(beginOrientation(p));
    expect(restarted.known).toEqual(['a']);
  });
...
describe('collecting targets and asking what the learner knows', () => {
  ...
  it('adds targets without dropping the ones already collected', () => {
    const p = plan();
    const first = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'add-targets', conceptIds: ['b', 'd'] });
    expect(first.run.targets).toEqual(['c', 'b', 'd']);
    const second = applyOrientationIntent(p, first.run, first.known, { kind: 'add-targets', conceptIds: ['b'] });
    expect(second.run.targets).toEqual(['c', 'b', 'd']);
  });

  it('spends a round of concepts whether or not the learner claims them', () => {
    const p = plan();
    const asked = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'ask-known', conceptIds: ['b', 'd'] });
    expect(asked.run.asked).toEqual(['b', 'd']);
    expect(asked.run.round).toBe(1);
    expect(asked.known).toEqual(['a']);
    const second = applyOrientationIntent(p, asked.run, asked.known, { kind: 'ask-known', conceptIds: ['c'] });
    expect(second.run.asked).toEqual(['b', 'd', 'c']);
    expect(second.run.round).toBe(2);
  });

  it('takes the concepts the learner claims, keeping only those the graph has', () => {
    const p = plan();
    const asked = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'ask-known', conceptIds: ['b', 'd'] });
    const claimed = applyOrientationIntent(p, asked.run, asked.known, { kind: 'know', conceptIds: ['b', 'ghost'] });
    expect(claimed.known).toEqual(['a', 'b']);
  });

  it('clears the rounds on a restart, so a second orientation asks from the top', () => {
    const p = plan();
    const asked = applyOrientationIntent(p, beginOrientation(p), ['a'], { kind: 'ask-known', conceptIds: ['b'] });
    expect(applyOrientationIntent(p, asked.run, asked.known, { kind: 'restart' }).run.asked).toEqual([]);
  });

  it('carries the known set through a step that does not touch it', () => {
    const p = plan();
    const moved = applyOrientationIntent(p, beginOrientation(p), ['a', 'b'], { kind: 'skip' });
    expect(moved.known).toEqual(['a', 'b']);
  });
});
