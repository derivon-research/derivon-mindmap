...
describe('replace with projection', () => {
  it('shows A B plus the shared C D subgraph on the points side', () => {
    ...
    expect(projection.concepts.map((concept) => concept.id)).toEqual(['A', 'B', 'C', 'D']);
    expect(projection.derivations.map((derivation) => derivation.id)).toEqual([
      'h-c',
      'h-a',
      'h-b',
      'h-d-points',
  ...
  it('shows X as an ordinary connected part of the shared C D graph', () => {
    ...
    expect(projection.concepts.map((concept) => concept.id)).toEqual(['C', 'D', 'X']);
    expect(projection.derivations.map((derivation) => derivation.id)).toEqual([
      'h-c',
      'h-x',
      'h-d-x',
    ...
    expect(projection.concepts.find((concept) => concept.id === 'X')?.controls).toEqual([{
      replaceWith: 'X',
      show: 'points',
      label: '2 点',
    }]);
  ...
  it('filters hidden-endpoint hyperedges instead of rewriting them to X', () => {
    ...
    expect(projection.derivations.every((derivation) => document.graph.derivations.includes(derivation))).toBe(true);
  });
  ...
  it('composes two view replacements recursively without synthetic nodes', () => {
    ...
    expect(projection.concepts.map((concept) => concept.id)).toEqual(['C', 'D']);
    expect(projection.concepts.find((concept) => concept.id === 'C')?.controls).toHaveLength(2);
