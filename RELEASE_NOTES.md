...
- Large graphs use progressive LOD: overview retains concept card silhouettes, then materializes labels, IDs, ports, derivation junctions, and edges for selected, hovered, focus, and route details.
...
- Layout uses a cancellable Worker with request IDs. Controlled graphs below 400 projected nodes use left-to-right Dagre; larger graphs use deterministic bipartite force plus an exact card-separation pass. Structural changes debounce for 120 ms and weight changes for 400 ms; edit relayout preserves the viewport.
